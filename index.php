...
$sql=$conn->prepare("SELECT username,passcode,Image,email,phno,gender from users where username=?");
...
$row = new StdClass;
	...
	$row->gender="";
	$row->email="";
    $sql->bind_result($row->id, $row->pass,$row->image,$row->email,$row->phno,$row->gender);
...
<img src="bg.jpg" width="500" height="500" style="position:absolute;left:20%;top:20%">
	...
	<p style="top:55%;left:85%;position:absolute">Not registered yet?<input type="button" onClick="document.location.href='adduser.html'" value="Signup">
</body>
</html>